...
This offers a more powerful way of extending the language than plain
macros.

At the same time, modifying the compiler in place turns out to be
really painful when building a compiler to implement a new language.
...
My current approach is to first allow compiler rules to be specified
and given names, and then a compiler can be constructed by supplying a
list of compiler rule names to be used for that compiler.
...
We pass through the variable name `x` unchanged, and recusively
...
Meanwhile, ssyntax *can* be implemented by extending the current
compiler.  (Adding ssyntax doesn’t prevent the compiler from compiling
code that it can already compile).  Thus:
...
Another issue is that when a container is first created, it doesn’t
contain any code (aside from references to the runtime builtins), and
specifically it doesn’t have a compiler.  Thus to load and eval code
it needs to make use of an already existing compiler defined somewhere
else.  However, if want to be able to extend the compiler
inside of a container like how ssyntax is implemented above, the
compiler would need to loaded in the container to be extended.  Thus
we need to be able to switch from using an external compiler to one
defined in the container.

Using `compile--xVrP8JItk2Ot` as a setting to specify which
compiler to use, `eval` is defined by:
...
that is, if the container specifies a compiler, we use the container’s
compiler, otherwise we use our own compiler on behalf of the container.

This may be trying too hard.  It does enable the compiler to be
extended dynamically within a container (generalizing the idea of
macros), but maybe it isn’t worth it.  An alternative would be to
create a compiler for a container including all of the compiler
features that you’ll be using, similar to how in Arc<sub>3.1</sub>
`arc.arc` loads with ssyntax already defined.
...
* Feature requests: if you might like to use Amacx as a compiler for
  your own Arc inspired langauge, please let me know what features it
